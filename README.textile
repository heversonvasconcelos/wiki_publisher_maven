...
Este plugin faz uso de macros que não são reconhecidas pelo Redmine por padrão.
Utiliza a função repo_include do plugin "scmacros":https://github.com/gpr/redmine_scmacros do Redmine.
Essa função inclúi um arquivo do repositório (svn, git, etc) na wiki do projeto.
Exemplo:
<pre>
